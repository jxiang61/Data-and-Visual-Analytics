...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        #############################################
        XX_length = len(XX)
        num_rows = np.random.choice(XX_length, n)
        
        for r in num_rows:
            labels.append(XX[r][-1])
            samples.append(XX[r][:-1])
        #############################################
    ...
    def fitting(self):
        ...
        #############################################
        par_node = {}
        num_decision_trees = len(self.decision_trees)
        for i in range(num_decision_trees):
            self.decision_trees[i].learn(self.bootstraps_datasets[i], self.bootstraps_labels[i], par_node, 0)
        #############################################
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for dt in self.decision_trees:
                    tmp_votes.append(dt.classify(record))
                
                y = np.append(y, np.argmax(np.bincount(tmp_votes)))
                #############################################
            else:
